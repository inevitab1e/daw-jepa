...
def main(args, resume_preempt=False):
    ...
    unsupervised_dataset, unsupervised_loader, unsupervised_sampler = make_imagenet1k(
        transform=transform,
    ...
    # -- DAW: difficulty-aware weighting config
    daw_cfg = args.get("daw", None)
    if daw_cfg is not None and daw_cfg.get("enabled", False):
        num_samples = len(unsupervised_dataset)
        difficulty_buffer = DifficultyBuffer(
            num_samples=num_samples,
            ema_alpha=daw_cfg.get("ema_alpha", 0.9),
            gamma=daw_cfg.get("gamma", 1.0),
            w_min=daw_cfg.get("w_min", 0.5),
            w_max=daw_cfg.get("w_max", 2.0),
            warmup_epochs=daw_cfg.get("warmup_epochs", 0),
            mode=daw_cfg.get("mode", "ema"),
            device=device,
        )
        logger.info("Difficulty-aware weighting enabled.")
    else:
        difficulty_buffer = None
        logger.info("Difficulty-aware weighting disabled.")
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for itr, (udata, masks_enc, masks_pred) in enumerate(unsupervised_loader):
            ...
            def load_imgs():
                ...
                indices = udata[1].to(device, non_blocking=True).long()

                masks_1 = [u.to(device, non_blocking=True) for u in masks_enc]
                masks_2 = [u.to(device, non_blocking=True) for u in masks_pred]
                return (imgs, masks_1, masks_2, indices)

            imgs, masks_enc, masks_pred, indices = load_imgs()
