...
def make_imagenet1k(
    ...
    if with_index:
        ...
        if subset_file is not None:
            dataset = ImageNetSubsetWithIndex(dataset, subset_file)
    else:
        dataset = ImageNet(
            root=root_path,
            image_folder=image_folder,
...
# 带着标号的
class ImageNetWithIndex(ImageNet):

    def __getitem__(self, index):
        # ImageFolder 的 samples 是一个 (path, target) 的 list
        path, target = self.samples[index]
